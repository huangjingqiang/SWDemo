...
import io.agora.openvcall.common.Constant;
import io.agora.openvcall.common.DynamicKey4;
...
import io.agora.propeller.ui.MyVideoCompositingLayout;
...
public class LiveRoomActivity extends BaseActivity implements AGEventHandler {
    ...
    private GridVideoViewLiveContainer mGridVideoViewContainer;

    private RelativeLayout mSmallVideoViewDock;

    private final HashMap<Integer, SurfaceView> mUidsList = new HashMap<>(); // uid = 0 || uid == EngineConfig.uid
    ...
    protected void initUIandEvent() {
        ...
        mGridVideoViewContainer = (GridVideoViewLiveContainer) findViewById(R.id.grid_video_view_container);
        mGridVideoViewContainer.setItemEventHandler(new VideoViewEventLiveListener() {
            @Override
        ...
        worker().joinLiveChannel(roomName, config().mUid, AGApplication.channelKey);
        TextView textRoomName = (TextView) findViewById(R.id.room_name);
        textRoomName.setText(roomName);
        findViewById(R.id.change_live_stream_layout).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (v.getTag() == null) {
                    if (setVideoCompositionLayout()) {
                        Toast.makeText(LiveRoomActivity.this, "切换画中画成功", Toast.LENGTH_LONG).show();
                    } else {
                        Toast.makeText(LiveRoomActivity.this, "切换画中画失败", Toast.LENGTH_LONG).show();
                    }

                    ;
                    v.setTag(1);
                } else {
                    if (clearVideoCompositingLayout()) {
                        Toast.makeText(LiveRoomActivity.this, "重置画中画成功", Toast.LENGTH_LONG).show();
                    } else {
                        Toast.makeText(LiveRoomActivity.this, "重置画中画失败", Toast.LENGTH_LONG).show();
                    }
                    v.setTag(null);
                }
            }
     ...
     * @return ；
     */
    private String getRecordingKey(String channelName, int uid) throws Exception {
        //getString(R.string.private_app_id), Constant.certificate,channelName,(int) new Date().getTime(),new Random().nextInt(1000),0
        String appId = getString(R.string.private_app_id);
        String appCertificate = Constant.certificate;
        int unixTs = (int) new Date().getTime();
        int randomInt = new Random().nextInt(1000);
        long uidL = uid;
        int expiredTs = 0;
        return DynamicKey4.generateRecordingKey(appId, appCertificate, channelName, unixTs, randomInt, uidL, expiredTs);
    }

    /**
     * 请求录制直播
     */
    private boolean recordLive(String recordKey) {
        return rtcEngine().startRecordingService(recordKey) == 0;
    }
     ...
     * @return ;
     */
    private boolean setVideoCompositionLayout() {
        MyVideoCompositingLayout layout = new MyVideoCompositingLayout();
        int i = 0;
        for (HashMap.Entry<Integer, SurfaceView> entry : mUidsList.entrySet()) {
            ...
            region.uid = entry.getKey();
            if (i == 0) {
                region.alpha = 1.0;
                region.height = layout.canvasHeight;
                region.width = layout.canvasWidth;
                region.renderMode = RENDER_MODE_HIDDEN;
                region.x = 0;
                region.y = 0;
                region.zOrder = i;
            } else {
                region.alpha = 0.8;
                region.height = layout.canvasHeight / 4;
                region.width = layout.canvasWidth / mUidsList.size();
                region.x = i * region.width;
                region.y = 0;
                region.zOrder = i;
                region.renderMode = RENDER_MODE_FIT;
            }
            layout.addRegion(region);
            i++;
        }
        return rtcEngine().setVideoCompositingLayout(layout) == 0;
    }
     ...
     * @return ;
     */
    private boolean clearVideoCompositingLayout() {
        return rtcEngine().clearVideoCompositingLayout() == 0;
    }
    ...
    private void doConfigEngine(int cRole) {
        ...
        worker().configEngineLive(cRole, vProfile);
    }
    ...
    public void onFirstRemoteVideoDecoded(int uid, int width, int height, int elapsed) {
        ...
        Log.d("Logger", "onFirstRemoteVideoDecoded: uid:" + uid + "is my uid:" + String.valueOf(uid == config().mUid));
    }
    ...
    public void onExtraCallback(int type, Object... data) {
        switch (type) {
            case EVENT_TYPE_ON_USER_JOINED:
                int uid = (int) data[0];
                int elapsed = (int) data[1];
                Log.d(TAG, "onExtraCallback: uid:" + uid + "elapsed:" + elapsed + "is my uid:" + String.valueOf(uid == config().mUid));
                break;
        }
    }
    ...
    private SmallVideoViewLiveAdapter mSmallVideoViewAdapter;
    ...
    private void bindToSmallVideoView(int exceptUid) {
        ...
        if (mSmallVideoViewAdapter == null) {
            create = true;
            mSmallVideoViewAdapter = new SmallVideoViewLiveAdapter(this, exceptUid, mUidsList, new VideoViewEventListener() {
                @Override
                public void onItemDoubleClick(View v, Object item) {
                    switchToDefaultVideoView();
                }
